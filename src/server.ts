      ...
      try {
        ...
        if (url.pathname === "/api/webhook/chapa" && request.method === "POST") {
          ...
          try {
            ...
            // Fetch booking first to check if we need to send an email
            const { data: bookingData } = await supabase.from("bookings").select("*").eq("ref", txRef).single();
            const booking = bookingData as any;

            const { error } = await supabase.from("bookings").update(updates).eq("ref", txRef);
            if (error) {
            ...
            if (booking && booking.status !== "confirmed" && newStatus === "confirmed") {
              try {
                ...
                console.error("Webhook email confirmation send error:", emailErr);
              }
            }
          } catch (e) {
